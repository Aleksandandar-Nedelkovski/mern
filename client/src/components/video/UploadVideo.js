...
const Catogory = [
  { value: 0, label: "Sleep" },
  { value: 0, label: "Nutrition" },
  { value: 0, label: "Physical Activity" },
];

function UploadVideoPage({ auth: { user } }) {
  const [title, setTitle] = useState("");
  const [Description, setDescription] = useState("");
  const [privacy, setPrivacy] = useState(0);
  const [Categories, setCategories] = useState("Sleep");
  ...
  const onSubmit = (event) => {
    ...
    if (
      ...
      return setAlert("Please first fill all the fields");
    }
  ...
  const onDrop = (files) => {
    ...
    axios.post("/api/videos/uploadfiles", formData, config).then((response) => {
      if (response.data.success) {
        ...
        axios
          ...
          .then((response) => {
            if (response.data.success) {
              ...
              setAlert("Failed to make the thumbnails");
            }
          })
          .catch((err) => {
            console.log(err);
        ...
        setAlert("Failed to save the video in server");
      }
    });
  };
