  ...
  async (req, res) => {
    ...
    if (lat && lng) {
      ...
      if (city !== "Kingston") {
        return res
          .status(400)
          .json({ msg: "You can only make crams in Kingston" });
      }
    } else {
    ...
    try {
      ...
      if (id) {
        ...
        if (!group) {
          ...
          return res
            .status(404)
            .json({ msg: "Event is bugged, does not have an owning group" });
        }
        ...
        const hostIndex = hosts
          .map((host) => host.user.toString())
          .indexOf(req.user.id);
        if (hostIndex < 0) {
          return res.status(401).json({
            msg: "You are not a host of the group that owns this event",
          });
        }
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    if (hostIndex < 0) {
      return res
        .status(401)
        .json({ msg: "You are not a host of the group that owns this event" });
    }
