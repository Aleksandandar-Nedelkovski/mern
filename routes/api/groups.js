...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    let requestingUser = group.members.filter(
...
router.delete("/posts/:group_id/:post_id", auth, async (req, res) => {
  try {
    ...
    // Pull out post
    const removeIndex = group.stickied_posts
      .map((post) => post.id)
...
router.put("/requests/:group_id", auth, async (req, res) => {
  try {
    ...
    if (!profile) {
      return res
        .status(401)
        .json({ msg: "Make a profile before joining groups" });
    }
    ...
    if (inviteIndex > -1) {
      ...
      return res.json({
        msg:
          "An invite was already sent to you, so we'll let you into the group",
      });
    }

    /* Check if they already sent a request */
    let existingRequest = group.requests.filter(
      (request) => request.toString() === req.user.id.toString()
    );
    if (existingRequest.length > 0) {
      return res
        .status(400)
        .json({ msg: "You have already sent a request to join this group" });
    }
...
router.delete("/requests/:group_id/:user_id", auth, async (req, res) => {
  try {
    ...
    const removeIndex = group.requests
...
router.put("/invite/:group_id/:profile_id", auth, async (req, res) => {
  try {
    ...
    /* Make sure the user isn't already in the group */
    let joiningUser = group.members.filter(
      (member) => member.user.toString() === profile.user.toString()
    );
    if (joiningUser.length > 0) {
    ...
    let existingInvite = profile.invites.filter(
      (invite) => invite.toString() === req.params.group_id
    );
    if (existingInvite.length > 0) {
      return res
        .status(400)
        .json({ msg: "This user already has a pending invite for this group" });
    }

    /* Check if they sent a request to join the group, just let them in */
    let requestIndex = group.requests
    ...
    if (requestIndex > -1) {
      ...
      await group.save();
      return res.json({
        msg: `${user.name} sent a request already so ${user.name} has been added to the group`,
      });
    }
...
router.put("/members/:group_id/:user_id", auth, async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ msg: "Profile Error, cannot retrieve user" });
    }
    ...
    if (requestIndex < 0) {
      return res
        .status(400)
        .json({ msg: "User did not request to join the group" });
    }

    /* send a push notification */
    sendLivePush(
      `${joiningProfile.user.toString()}`,
      `You have been accepted into ${group.name}`
    );
...
router.put("/hostpriv/:group_id/:profile_id", auth, async (req, res) => {
  try {
    ...
    if (memberIndex !== 0) {
      ...
      return res
        .status(401)
        .json({ msg: "Founding host cannot unhost themself" });
    }
